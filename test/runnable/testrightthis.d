...
{
    int v;
    S!v s;  // Error: need 'this' to access member v (with 2.069.2)
            // Error: field must be initialized in constructor (with fix15734)
...
//    c.v = 2;
    ...
    // --> C3's ctor (autogenerated or not) should initialize S with S()
    // --> which its hidden field is set to C3's 'this' reference.
//    assert(c.s.tupleof[$-1] is null);
}

struct S4
{
    int v;
    S!v s;  // Error: need 'this' to access member v (with 2.069.2 & fix15734(-o- is no error, but -c will be "need 'this' to access member v"))
...
void test4()
{
    static assert(!__traits(compiles, S4()));
    static assert(!__traits(compiles, S4(1)));
    //auto s3 = S4(1, S!(S4.v)()); // ICE, typeof(S4.s) != S!(S4.v) in here!
}

template X(alias a)
{
    S!a s;  // --> error: cannot default constructible
}

void test5()
{
    int v = 2;
    static assert(!__traits(compiles, X!v));
   // alias x = X!v;
   // //assert(x.s.foo() == 6);   // there's no timing to initialize the hidden field
   // assert(x.s.tupleof[$-1] is null);
