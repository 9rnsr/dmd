...
Type *Type::wildConstOf()
{
    ...
    if (mod == (MODwild | MODconst))
        return this;
    if (wcto)
    {
        assert(wcto->mod == (MODwild | MODconst));
        return wcto;
    }
...
Type *Type::sharedWildConstOf()
{
    ...
    if (mod == (MODshared | MODwild | MODconst))
        return this;
    if (swcto)
    {
        assert(swcto->mod == (MODshared | MODwild | MODconst));
        return swcto;
    }
...
#define X(m, n) (((m) << 4) | (n))
    switch (mod)
    {
        ...
        case MODshared | MODwild | MODconst:
            swcto = mto;
            t->swcto = this;
            break;
...
void Type::check()
{
    switch (mod)
    {
        case MODimmutable:
            ...
            if (wcto) assert(wcto->mod == (MODwild | MODconst));
            if (swto) assert(swto->mod == (MODshared | MODwild));
            if (swcto) assert(swcto->mod == (MODshared | MODwild | MODconst));
            break;
        ...
        case MODwild | MODconst:
            ...
            assert(!swcto || swcto->mod == (MODshared | MODwild | MODconst));
            break;
        ...
        case MODshared | MODwild | MODconst:
            ...
            assert(! scto ||  scto->mod == (MODshared | MODconst));
            assert(!  wto ||   wto->mod == MODwild);
            assert(! wcto ||  wcto->mod == (MODwild | MODconst));
            assert(! swto ||  swto->mod == (MODshared | MODwild));
...
Type *Type::makeSharedWildConst()
{
    ...
    Type *t = this->nullAttributes();
    t->mod = MODshared | MODwild | MODconst;
    return t;
}
...
unsigned char MODmerge(unsigned char mod1, unsigned char mod2)
{
    ...
    else
    {
        ...
        // MODimmutable vs MODwild | MODconst
        //      MODwild vs MODwild | MODconst
        assert(mod1 & MODwild || mod2 & MODwild);
        result |= MODwild | MODconst;
    }
    return result;
}
