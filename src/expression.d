...
/***********************************************************
 * Don't be generated by parser.
 * A placeholder object to call DsymbolExp.resolve on specific symbol.
 */
...
public:
    ...
    Lagain:
        ...
        if (auto f = s.isFuncDeclaration())
        {
            ...
            // Because nested functions and function literals cannot have overloads.
            keepOverloads &= !f.isNested() && !f.isFuncLiteralDeclaration();

            if (!keepOverloads && f.checkForwardRef(loc))
                return new ErrorExp();

            FuncDeclaration fd = s.isFuncDeclaration();
            fd.type = f.type;
            return new VarExp(loc, fd, keepOverloads);
        }
 ...
 * Offset from symbol
 *
 * Assume that the Declaration symbol `var` has no ambiguity. So
 * `SymbolExp.hasOverloads` is `true` always.
 */
...
public:
    ...
    {
        super(loc, TOKsymoff, __traits(classInstanceSize, SymOffExp), var, false/*hasOverloads*/);
        this.offset = offset;
...
public:
    ...
    Lagain:
        ...
        else
        {
            ...
            //{
            //    // --> overloaded function symbol is left as VarExp
