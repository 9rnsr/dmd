...
bool checkDefCtor(Loc loc, Type *t)
{
    ...
    if (t->ty == Tstruct)
    {
        ...
        // TODO: Should also reject nested structs?
    }
    return false;
}
#endif
...
Ltupleassign:
        ...
        if (op == TOKconstruct)
        {
            ...
            if (t2->ty == Tstruct && sd == ((TypeStruct *)t2)->sym)
            {
                    ...
                    e2->type->implicitConvTo(t1))
                {
                    ...
                    dvx->e1 = this->e1; // TODO: this->e1 may have side effect when this
                                        // is a field initializing inside constructor?
            ...
            else if (!e2->implicitConvTo(t1))
            {
                if (sd->ctor)
                {
                    ...
                    ex->op = TOKblit;
                    ex->type = e1->type;
                    // TODO: May not cover nested struct construction?
    ...
    else if (t1->ty == Tsarray)
    {
            ...
            ((IndexExp *)e1)->e1->type->toBasetype()->ty == Taarray)
        {
            ...
            if (e2->op == TOKerror)
                return e2;
            // TODO: why here's case is specially handled?
        }
        else if (op == TOKconstruct)
        {
            ...
            if (e2x->op == TOKslice)
            {
                ...
                {
                    // TODO: can move this handling in SliceExp::inferType(t1) ?
                    e2x = se->e1;
                }
            }
    ...
    else if (t1->ty == Tsarray)
    {
        ...
        {
            assert(op != TOKassign);
            // TODO: may move to here the implicitCastTo call on (AA indexing / NRVO call)?
        }
             ...
             t2->nextOf()->implicitConvTo(t1->nextOf()))
    {
        ...
        if (op == TOKconstruct)
        {
            ...
            /* BUG:
             *  int[3] sa;
             *  uint[3] ua = sa;    // should be rejected
             *  ua = sa;            // rejected, OK
             */
        }
        else
