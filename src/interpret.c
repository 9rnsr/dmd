...
public:
    ...
    */
    ...
    void visit(NewExp *e)
    {
        ...
        if (e->argprefix)
        {
            Expression *ex = interpret(e->argprefix, istate, ctfeNeedNothing);
            if (exceptionOrCantInterpret(ex))
                return;
        }
        ...
        if (e->newtype->toBasetype()->ty == Tstruct)
        {
            ...
            else
            {
                ...
                if (e->arguments)
                {
                    ...
                    {
                        Expression *ex = interpret((*e->arguments)[i], istate);
                        if (exceptionOrCantInterpret(ex))
                            return;
                        (*exps)[i] = ex;
                    }
    ...
    #endif
        Expression *ex1 = interpret(e->e1, istate);
        assert(ex1);
        if (exceptionOrCantInterpret(ex1))
            return;
    ...
    void visit(IndexExp *e)
    {
        ...
        if (e->e1->type->toBasetype()->ty == Tpointer)
        {
            ...
            Expression *ex2 = interpret(e->e2, istate);
