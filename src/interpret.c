...
//printf("\tL%d\n", __LINE__);
    ...
    void visit(TupleExp *e)
    {
        ...
        if (CTFEExp::isCantExp(interpret(e->e0, istate)))   // can be evaluated with ctfeNeedNothing
        {
            result = CTFEExp::cantexp;
            return;
        }
...
//printf("\tdst = %s, src = %s\n", tdst->toChars(), tsrc->toChars());
        ...
        /* Evaluate e1
         * Some special assignment forms will be handled later.
         */
//printf("+L%d [%s] e1 = %s\n", __LINE__, e->loc.toChars(), e1->toChars());
        ...
        else if (e1->op == TOKslice/*isBlockAssignment || isElementWiseAssignment*/)
        {
            e1 = interpret(e1, istate, ctfeNeedRvalue);
            if (exceptionOrCant(e1))
                return;
            // Validate CTFE-able slices
            //printf("L%d [%s] e = %s, e1 = %s\n", __LINE__, e->loc.toChars(), e->toChars(), e1->toChars());
            assert(e1->op == TOKnull || e1->op == TOKarrayliteral || e1->op == TOKstring || e1->op == TOKslice);
            assert(e1->type->toBasetype()->ty == Tarray);
        }
        else
        {
//printf("+L%d [%s] e1 = %s\n", __LINE__, e->loc.toChars(), e1->toChars());
            // evaluate e1 as a CTFEable lvalue (CTFEable reference).
            e1 = interpret(e1, istate, ctfeNeedLvalue);
            if (exceptionOrCant(e1))
                return;

        #if 1   // Validate CTFE-able lvalues (CTFEable reference)
            if (e1->op == TOKvar)
            {
            #if 0
                Expression *eval = getValue(((VarExp *)e1)->var);
                assert(eval != NULL);
                assert(eval->op == TOKint64 || eval->op == TOKfloat64 || eval->op == TOKcomplex64 ||
                        ...);
            #endif
            }
            else if (e1->op == TOKdotvar)
            {
                ...
                //printf("L%d [%s] e = %s, dve->e1 = %s\n", __LINE__, e->loc.toChars(), e->toChars(), dve->e1->toChars());
                assert(dve->e1->op == TOKstructliteral || dve->e1->op == TOKclassreference);
                if (dve->e1->op == TOKstructliteral)
                    assert(((StructLiteralExp *)dve->e1)->ownedByCtfe);
                if (dve->e1->op == TOKclassreference)
                    assert(((ClassReferenceExp *)dve->e1)->value->ownedByCtfe);
            }
            else if (e1->op == TOKindex)
            {
                Expression *eagg = ((IndexExp *)e1)->e1;
                //printf("L%d [%s] e = %s, eagg = %s\n", __LINE__, e->loc.toChars(), e->toChars(), eagg->toChars());
                assert(eagg->op == TOKarrayliteral || eagg->op == TOKstring || eagg->op == TOKassocarrayliteral);
            }
            else if (e1->op == TOKarrayliteral)
            {
                assert(e1->type->toBasetype()->ty == Tsarray);
                assert(((ArrayLiteralExp *)e1)->ownedByCtfe);
            }
            else if (e1->op == TOKstring)
            {
                assert(e1->type->toBasetype()->ty == Tsarray);
                assert(((StringExp *)e1)->ownedByCtfe);
            }
            else if (e1->op == TOKstructliteral)
            {
                assert(((StructLiteralExp *)e1)->ownedByCtfe);
            }
            else
            {
                //printf("L%d [%s] e = %s, e1 = %s\n", __LINE__, e->loc.toChars(), e->toChars(), e1->toChars());
...
//printf("-L%d [%s] e1 = %s\n", __LINE__, e->loc.toChars(), e1->toChars());
        ...
        Expression *newval = interpret(e->e2, istate, isRefAssign ? ctfeNeedLvalue : ctfeNeedRvalue);
...
//if (e1->op == TOKdotvar) printf("L%d [%s] e1: dve->e1 = %p %s\n", __LINE__, e->loc.toChars(), ((DotVarExp *)e1)->e1, ((DotVarExp *)e1)->e1->toChars());
#if 0   // Now 'reference types' handling is not special, so this branch would be unnecessary.
        // ---------------------------------------
...
//printf("L%d [%s] e1 = %s, newval = %s\n", __LINE__, e->loc.toChars(), e1->toChars(), newval->toChars());
        ...
        else if (e1->op == TOKslice || e1->op == TOKarrayliteral || e1->op == TOKstring || e1->op == TOKnull && e1->type->toBasetype()->ty == Tarray)
        {
...
//printf("L%d [%s] e1 = %s, newval = %s (blockAssign = %d)\n", __LINE__, e->loc.toChars(), e1->toChars(), newval->toChars(), isBlockAssignment);
            ...
            if (e1->op == TOKslice)
                ...
                se = new SliceExp(e1->loc, e1, NULL, NULL);
...
//printf("L%d [%s] aggregate = %s, newval = %s\n", __LINE__, ie->loc.toChars(), aggregate->toChars(), newval->toChars());
        ...
        if (!wantRef && newval->op == TOKslice)
        {
            ...
            if (CTFEExp::isCantExp(newval))
            {
                originalExp->error("Internal Compiler Error: CTFE index assign %s", originalExp->toChars());
                assert(0);
            }
        }
...
//printf("L%d [%s] wantRef = %d, aggregate = %s, newval = %s\n", __LINE__, originalExp->loc.toChars(), wantRef, aggregate->toChars(), newval->toChars());
        if (!wantRef && newval->op == TOKslice)
        {
            ...
            if (CTFEExp::isCantExp(newval))
            {
                originalExp->error("Internal Compiler Error: CTFE slice %s", orignewval->toChars());
                assert(0);
            }
        }
        if (wantRef && newval->op == TOKindex &&
...
//printf("L%d e->e1 = %s\n", __LINE__, e->e1->toChars());
        Expression *ethis = interpret(e->e1, istate);
        if (exceptionOrCant(ethis))
            return;
//assert(ethis);
//printf("L%d ethis = %d %d %d %d %d\n", __LINE__, ethis == CTFEExp::cantexp, ethis == CTFEExp::voidexp, ethis == CTFEExp::breakexp, ethis == CTFEExp::continueexp, ethis == CTFEExp::gotoexp);
...
//printf("L%d [%s] ethis = %p %s\n", __LINE__, e->loc.toChars(), ethis, ethis->toChars());
        ...
        if (ethis->op == TOKnull)
        {
            if (ethis->type->toBasetype()->ty == Tclass)
        ...
        if (ethis->op != TOKstructliteral && ethis->op != TOKclassreference)
        {
        ...
        if (ethis->op == TOKclassreference)
        {
            se = ((ClassReferenceExp *)ethis)->value;
            i  = ((ClassReferenceExp *)ethis)->findFieldIndexByName(v);
        }
        else
        {
            se = (StructLiteralExp *)ethis;
        ...
        if (goal == ctfeNeedLvalue/* || goal == ctfeNeedLvalueRef*/)
        {
        #if 1   // Need to avoid compilable/interpret3.d breaking
...
//printf("\t--> L%d\n", __LINE__);
            ...
            {
                result = new DotVarExp(e->loc, ethis, v);
                result->type = e->type;
            }
//printf("\t--> L%d ethis = %p %s, result = %s\n", __LINE__, ethis, ethis->toChars(), result->toChars());
            return;
        }

        // If it is an rvalue literal, return it...
        if (result->op == TOKvoid)  // VoidInitExp represents uninitialized data
        {
        ...
        if (result->op == TOKstructliteral ||
            result->op == TOKarrayliteral ||
            result->op == TOKassocarrayliteral ||
            result->op == TOKstring)
        {
            // might need to repaint literal type by e->type?
//printf("\t--> L%d\n", __LINE__);
            return;
        }
        if (isPointer(e->type)) // -> isCtfePointer(e) ?
        {
            result = paintTypeOntoLiteral(e->type, result);
//printf("\t--> L%d\n", __LINE__);
            return;
        }
        // e->var might be STCref ?
        if (result->op == TOKvar)
        {
    ...
    #endif
        Expression *eagg = interpret(e->e1, istate);
        if (exceptionOrCant(eagg))
            return;
        Expression *eindex = interpret(e->e2, istate);
        if (exceptionOrCant(eindex))
            return;

        if (eagg->op == TOKnull)
        {
            // 'remove' does nothing for the non existing index
            //result = CTFEExp::voidexp;
            result = new IntegerExp(e->loc, 0, Type::tbool);  // bugfix
            return;
        }

        assert(eagg->op == TOKassocarrayliteral);
        AssocArrayLiteralExp *aae = (AssocArrayLiteralExp *)eagg;
        Expressions *keys = aae->keys;
        Expressions *vals = aae->values;
        size_t removed = 0;
        for (size_t j = 0; j < vals->dim; ++j)
        {
            Expression *ekey = (*keys)[j];
            if (ctfeEqual(e->loc, TOKequal, ekey, eindex))
            {
                ++removed;
            }
            else if (removed > 0)
            {
                (*keys)[j - removed] = ekey;
                (*vals)[j - removed] = (*vals)[j];
            }
        }
        vals->dim = vals->dim - removed;
        keys->dim = keys->dim - removed;
