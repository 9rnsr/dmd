...
//printf("\tL%d [%s] e = %s\n", __LINE__, e->loc.toChars(), e->toChars());
        ...
        if (e1->op == TOKindex)
        {
            if (!interpretAssignToIndex(e->loc, istate, (IndexExp *)e1, newval, e))
            {
                result = CTFEExp::cantexp;
            }
            return;
     ...
     */
    static bool interpretAssignToIndex(Loc loc, InterState *istate,
        IndexExp *ie, Expression *newval,
        BinExp *originalExp)
    {
    ...
    void visit(IndexExp *e)
    {
        ...
        if (agg->op != TOKarrayliteral && agg->op != TOKstring)
            printf("[%s] index agg = %s\n", e->loc.toChars(), agg->toChars());
        result = ctfeIndex(e->loc, e->type, agg, indexToAccess);
        if (exceptionOrCant(result))
            return;
        result = paintTypeOntoLiteral(e->type, result);
