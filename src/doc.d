...
public:
    override void write(Loc loc, DocComment* dc, Scope* sc, Dsymbols* a, OutBuffer* buf)
    {
        ...
        DocComment.parseMacros(dc.pescapetable, dc.pmacrotable, bodystr, bodylen);
    }
}
...
extern (C++) void gendocfile(Module m)
{
    ...
    if (dc.copyright)
    {
        ...
        Macro.define(&m.macrotable, cast(char*)"COPYRIGHT", 9, dc.copyright.bodystr, dc.copyright.bodylen);
    }
    ...
    extern (C++) void writeSections(Scope* sc, Dsymbols* a, OutBuffer* buf)
    {
        ...
        for (size_t i = 0; i < sections.dim; i++)
        {
            ...
            if (!sec.namelen && i == 0)
            {
                ...
                size_t o = buf.offset;
                buf.write(sec.bodystr, sec.bodylen);
                escapeStrayParenthesis(loc, buf, o);
                highlightText(sc, a, buf, o);
                buf.writestring(")\n");
            }
