        ...
        Ldefault:
            ...
            if (op != TOKblit)
            {
                ...
                {   /* Need to do postblit.
                    ...
                    Expression *ti = getTypeInfo(NULL, tb);
...
elem *toElem(Expression *e, IRState *irs)
{
    ...
    public:
        ...
        void visit(SymbolExp *se)
        {
            ...
            {
                result = el_long(se->type->totym(), 0);
                return;
            }
            ...
            if (v && v->offset)
            {
                ...
                if (se->op == TOKvar)
                {
                    e = el_una(OPind, se->type->totym(), e);
        ...
        {
            elem *e = el_long(ie->type->totym(), ie->value);
            el_setLoc(e,ie->loc);
            result = e;
        }
        ...
        void visit(RealExp *re)
        {
            ...
            union eve c;
            memset(&c, 0, sizeof(c));
            tym_t ty = re->type->toBasetype()->totym();
            switch (tybasic(ty))
            {
                case TYfloat:
                case TYifloat:
        ...
        {
            result = el_long(ne->type->totym(), 0);
        }
        ...
        void visit(NewExp *ne)
        {
            ...
            else if (t->ty == Tpointer && t->nextOf()->toBasetype()->ty == Tstruct)
            {
                ...
                else
                {
                    ...
                    // call _d_newitemT(ti)
                    e = getTypeInfo(NULL, ne->type)->toElem(irs);
            ...
            else if (t->ty == Tarray)
            {
                ...
                if (ne->arguments->dim == 1)
                {
                    ...
                    // call _d_newT(ti, arg)
                    e = el_param(e, getTypeInfo(NULL, ne->type)->toElem(irs));
        ...
        void visit(NegExp *ne)
        {
            ...
            switch (tb1->ty)
            {
                case Tvector:
                {
                    ...
                    ez->EV.Vcent.lsw = 0;
                    ez->EV.Vcent.msw = 0;
                    e = el_bin(OPmin, ne->type->totym(), ez, e);
                    break;
                }

                default:
                    e = el_una(OPneg, ne->type->totym(), e);
                    break;
            }
        ...
        {
            elem *e = el_una(OPnot, ne->type->totym(), ne->e1->toElem(irs));
            el_setLoc(e,ne->loc);
            result = e;
        }
        ...
        void visit(CatExp *ce)
        {
            ...
            if (ce->e1->op == TOKcat)
            {
                ...
                ep = el_params(ep,
                               el_long(TYsize_t, n),
                               getTypeInfo(NULL, ta)->toElem(irs),
                               NULL);
            ...
            else
            {
                ...
                elem *ep = el_params(e2, e1, getTypeInfo(NULL, ta)->toElem(irs), NULL);
        ...
        void visit(EqualExp *ee)
        {
            ...
            else if (t1->ty == Taarray && t2->ty == Taarray)
            {
                ...
                elem *ti = getTypeInfo(NULL, taa)->toElem(irs);
        ...
        void visit(IdentityExp *ie)
        {
                     ...
                     (t2->ty == Tarray || t2->ty == Tsarray))
            {
                ...
                e = el_bin(eop, ie->type->totym(), ea1, ea2);
                el_setLoc(e, ie->loc);
            }
            else
                e = toElemBin(ie, eop);
        ...
        void visit(InExp *ie)
        {
            ...
            elem *e = el_bin(OPcall, ie->type->totym(), el_var(s), ep);
            ...
            Lx:
                ...
                else
                {
                    ...
                    else if (postblit && ae->op != TOKblit)
                    {
                         ...
                         */
                        el_free(esize);
                        Expression *ti = getTypeInfo(NULL, t1->nextOf()->toBasetype());
                        if (config.exe == EX_WIN64)
                        {
                        ...
                        e = el_bin(OPcall, ae->type->totym(), el_var(rtlsym[rtl]), ep);
                    }
                    else
                    {
                        // Generate:
                        ...
                        e = el_bin(OPcall, ae->type->totym(), el_var(rtlsym[RTLSYM_ARRAYCOPY]), ep);
                    }
                    el_setLoc(e, ae->loc);
                    result = e;
                    return;
            ...
            else if (tb1->ty == Tarray || tb2->ty == Tsarray)
            {
                    ...
                    tb1n->equals(tb2->nextOf()->toBasetype()))
                {
                    ...
                    if (config.exe == EX_WIN64)
                        ...
                        e2 = useOPstrpar(e2);
                    elem *ep = el_params(e2, e1, getTypeInfo(NULL, ce->e1->type)->toElem(irs), NULL);
                ...
                else if (tb1n->equals(tb2))
                {
                    ...
                    e1 = el_una(OPaddr, TYnptr, e1);
                    elem *ep = el_param(e1, getTypeInfo(NULL, ce->e1->type)->toElem(irs));
        ...
        void visit(PowAssignExp *e)
        {
            ...
            result = el_long(e->type->totym(), 0);  // error recovery
        }
        ...
        void visit(UshrExp *se)
        {
            ...
            elem *e = el_bin(OPshr, se->type->totym(), eleft, eright);
            el_setLoc(e, se->loc);
            result = e;
        }
        ...
        void visit(DotVarExp *dve)
        {
            ...
            e = el_una(OPind, dve->type->totym(), e);
            if (tybasic(e->Ety) == TYstruct)
            {
        ...
        void visit(PtrExp *pe)
        {
            ...
            elem *e = pe->e1->toElem(irs);
            e = el_una(OPind,pe->type->totym(),e);
            if (tybasic(e->Ety) == TYstruct)
            {
        ...
        void visit(BoolExp *e)
        {
            ...
            result = el_una(OPbool, e->type->totym(), e1);
        }
            ...
            switch (tb->ty)
            {
                case Tarray:
                {
                    ...
                    if (tv->ty == Tstruct)
                    {
                        ...
                        if (sd->dtor)
                            et = getTypeInfo(NULL, tb->nextOf())->toElem(irs);
                    }
        ...
        void visit(CastExp *ce)
        {
            ...
            if (tty == Tarray && fty == Tarray)
            {
                ...
                {   // Array element sizes do not match, so we must adjust the dimensions
                    ...
                    e = el_bin(OPcall, ce->type->totym(), el_var(rtlsym[RTLSYM_ARRAYCAST]), ep);
                }
                goto Lret;
            }
        ...
        void visit(ArrayLengthExp *ale)
        {
            ...
            e = el_una(I64 ? OP128_64 : OP64_32, ale->type->totym(), e);
            el_setLoc(e, ale->loc);
            result = e;
        }
        ...
        void visit(SliceExp *se)
        {
            ...
            if (se->lwr)
            {
                ...
                else
                {
                    ...
                    e = el_una(OPind, se->type->totym(), eptr);
                    if (tybasic(e->Ety) == TYstruct)
                        e->ET = Type_toCtype(se->type);
                }
                e = el_combine(elwr, e);
        ...
        void visit(IndexExp *ie)
        {
            ...
            if (t1->ty == Taarray)
            {
                ...
                //printf("taa->index = %s\n", taa->index->toChars());
                elem* keyti = taa->index->getInternalTypeInfo(NULL)->toElem(irs);
                //keyti = getTypeInfo(NULL, taa->index)->toElem(irs);
                //printf("keyti:\n");
                //elem_print(keyti);
                ...
                e = el_una(OPind, ie->type->totym(), e);
                if (tybasic(e->Ety) == TYstruct)
        ...
        void visit(ArrayLiteralExp *ale)
        {
            ...
            else if (ale->elements)
            {
                ...
                e = el_param(e, getTypeInfo(NULL, ale->type)->toElem(irs));
