...
#if 0
    printf("checkAccessX for %s.%s in function %s() in scope %s\n",
        ...
        if (ClassDeclaration *cdthis = dthis->isClassDeclaration())
        {
            for (size_t i = 0; i < cdthis->baseclasses->dim; i++)
            {
                ...
                if (access.kind >= PROTprotected ||
                    checkAccessX(smember, sfunc, b->base, cdscope))
                {
                    return true;
                }
            }
        ...
        if (smember->toParent() != dthis)
        {
            ...
            if (cdthis)
            {
                ...
                {   BaseClass *b = (*cdthis->baseclasses)[i];
                    ...
                    if (checkAccessX(smember, sfunc, b->base, cdscope))
                        return true;
                }
            }
        }
...
void checkAccess(AggregateDeclaration *ad, Loc loc, Scope *sc, Dsymbol *smember)
{
    ...
    {
        result = checkAccessX(smember, f, ad, cdscope);
#if LOG
        printf("result4 = %d\n", result);
#endif
    }
