...
Type *Initializer::checkMultiDimInit(Scope *sc, Type *t)
{
    ...
    if (tb->ty == Tsarray)
    {
        ...
        if (isArrayInitializer() &&
            tn->ty != Tarray && tn->ty != Tsarray && tn->ty != Taarray/* && tn->ty != Tpointer*/)
        {
...
bool ArrayInitializer::canMatch(Scope *sc, Type *t)
{
    ...
    if (t->ty == Tarray || t->ty == Tsarray || t->ty == Taarray/* || t->ty == Tpointer*/)
    {
        if (value.dim)
        {
...
Initializer *ArrayInitializer::semantic(Scope *sc, Type *t)
{
    ...
    if (t->ty == Tsarray)
    {
        bool needInterpret = (sc->flags & SCOPEctfe);
